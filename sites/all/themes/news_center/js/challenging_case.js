		...
		var object = {"type":"votes", "single":"0", "criteria":{"entity_type":"node", "entity_id":nid, "uid":uid}};
			...
			$.ajax({
				...
				success: function ( data ) {
					...
					if(Drupal.settings.challengingcase.uid >= 1){
		...
		$.each(comments, function(index,value){
			...
			if(typeof comments[index].comment_body != "undefined"){
				...
				if(pid == comments[index].pid){
					...
					html += '<div id="comment-wrapper-'+comments[index].cid+'">'+
...
*/
	...
	function renderVotingResults(result){
		...
		$(".cc_row_voting_changevote").click(function(){changeVoteClicked(Drupal.settings.challengingcase.nid,Drupal.settings.challengingcase.uid)});
	...
	function changeVoteClicked(nid,uid){
		...
		vote(Drupal.settings.challengingcase.nid,Drupal.settings.challengingcase.uid,ccuser_vote==1?0:1);
	...
	function refreshVotingButtons(){
		$("#cc_vote_pro").click(function(){
			...
			if(Drupal.settings.challengingcase.uid <= 0){
				window.location.href = '/user/login';

			}else{
				vote(Drupal.settings.challengingcase.nid,Drupal.settings.challengingcase.uid,1);
				$(".cc_vote_btn").hide();
				$(".cc_kol_voting").show();
			}
	...
	$(document).ready( function () {
		...
		getVoteResultForUser(Drupal.settings.challengingcase.nid,Drupal.settings.challengingcase.uid);
				...
				$(".comment-add A").click(function(){
						...
						$("#comments > .comment-form input[type=submit]").mousedown(function(){
							var comment = $("#comments > .comment-form .form-textarea").val();
								...
								addComment(Drupal.settings.challengingcase.nid,Drupal.settings.challengingcase.uid, 0, "",  comment);
