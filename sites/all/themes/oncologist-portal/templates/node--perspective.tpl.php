<?php 
	...
	$nid = $node->nid;
	$title = $node->title;
	$author_image = $node->field_perspective_author_picture[$lang][0]['filepath'];
	...
	$thumnail =  $node->field_perspective_mediathumbnail[$lang][0]['filepath'];
	$streaming = $node->field_perspective_murl_streamed[$lang][0]['value'];
	$direct = $node->field_perspective_direct[$lang][0]['value'];
	$isaudio = $node->field_perspective_isaudio[$lang][0]['value'];
	$bc_id = $node->field_bcexport[$lang][0]['video_id'];
	
	$article = isset($node->field_perspective_htmlfile[$lang][0]['filepath']) ? $node->field_perspective_htmlfile[$lang][0]['filepath'] : "";
	...
	<div id="share-box">
		...
		<!-- AddThis Button END -->
	     ...
	     <div id="tabs" class="cc_tabs">
					...
					<div class="cc_tab buttons"><a <?php print $page == "perspective" ? "class=\"selected\"" : "";?>  id="perspective_btn" href="#perspective" onclick="return showtab('perspective');">Perspective</a></div>
					
					<?php
						if($article != ""):
					?>
					<div class="cc_tab buttons"><a <?php print $page == "article" ? "class=\"selected\"" : "";?>  id="article_btn" href="#article" onclick="return showtab('article');">Article</a></div>
					<?php		
						endif;
					?>
...
function showtab(id) {
  ...
  $(".eup_panels").hide();
  $("#tabs .buttons A").removeClass("selected");
  ...
  if(id == "perspective"){
  	...
  	$("#discussion").show();
  	$("#discussion_btn").addClass("selected");
  }
  //myScroll.refresh();
...
<?php 
	...
	print 'createUser('.$uid.',"'.$first.'","'.$last.'", "'.$user->mail.'")';
